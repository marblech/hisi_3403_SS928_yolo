...
void draw_boxes(cv::Mat& image, const std::vector<YOLOV11::bbox_t>& boxes) {
    for(size_t i = 0; i < boxes.size(); i++) {        
        ...
        putText(image, std::to_string(boxes[i].obj_id), cv::Point(boxes[i].rect.x, boxes[i].rect.y), cv::FONT_HERSHEY_PLAIN, 2.0, cv::Scalar(255, 255, 0), 2, 8);
    }
}

int main(){
    YOLOV11 yolo;
    std::string model_path = "/app/marble_chen/yolov5_lib_test/model/yolo11n.om";
    ...
    std::vector<YOLOV11::bbox_t> result_list;
